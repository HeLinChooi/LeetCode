# LeetCode

Solutions, one file per problem.

The notes that go with them are not here. They are in the knowledge base, under
`docs/interviews/`, and they read better on a phone than a `.cpp` file does:

- **Interview Prep Plan** — what to do this week, and why that
- **Patterns & Lessons** — the ideas that outlast the problem that taught them
- **LeetCode Prep Log** — which problems went badly, and when they are due again

This repo is a source for that site. Any `.md` written here is published at
`/leetcode/` on the next build, read straight from this folder — so there is
never a second copy to keep in step.
